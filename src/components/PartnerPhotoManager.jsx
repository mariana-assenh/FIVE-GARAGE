// @ts-nocheck
// Gerenciador de fotos de um anúncio parceiro — idêntico ao
// VehiclePhotoManager (mesmo limite de 20 fotos / 200MB, drag-and-drop,
// capa, progresso), só que apontando pra src/lib/partnerListingPhotos.js
// (bucket/tabela dos parceiros) em vez dos veículos.
...
import {
  ...
  insertPartnerListingPhotoRows,
  deletePartnerListingPhoto,
  deletePartnerListingPhotoFile,
  setPartnerCoverPhoto,
  reorderPartnerListingPhotos,
} from "@/lib/partnerListingPhotos";
...
export default function PartnerPhotoManager({
  mode, // "draft" | "persisted"
  listingId = null, // obrigatório quando mode === "persisted"
  folder, // prefixo usado no Storage (draftId ou listingId)
  photos,
  onPhotosChange,
  disabled = false,
}) {
  ...
  const handleFilesSelected = async (fileList) => {
    ...
    for (let i = 0; i < validFiles.length; i++) {
      ...
      try {
        ...
        const uploaded = await uploadPartnerListingPhoto(compressed, {
          folder,
        ...
        if (mode === "persisted") {
          ...
          const [inserted] = await insertPartnerListingPhotoRows(listingId, [
  ...
  const handleDelete = async (photo) => {
    ...
    try {
      if (photo.id) {
        await deletePartnerListingPhoto(photo);
      } else if (photo.storage_path) {
        await deletePartnerListingPhotoFile(photo.storage_path);
      }
  ...
  const handleSetCover = async (photo) => {
    ...
    try {
      if (mode === "persisted" && photo.id) {
        await setPartnerCoverPhoto(photo.id);
      }
  ...
  const handleDragEnd = async (result) => {
    ...
    if (mode === "persisted") {
      ...
      try {
        await reorderPartnerListingPhotos(ids);
      } catch (err) {
    ...
    <div className="space-y-3">
      ...
      {photos.length > 0 && (
        <DragDropContext onDragEnd={handleDragEnd}>
          <Droppable droppableId="partner-photos" direction="horizontal">
            {(provided) => (
